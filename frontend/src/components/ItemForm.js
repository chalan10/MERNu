...
function ItemForm({ menu, setMenu, menuCategory, menuItem, toggleAddItem, toggleEditItem, deleteItem, handleItemSubmit }) {
		...
		return (
			<form
				...
				onSubmit={(e) => handleItemSubmit(e, {
					catid: menuCategory._id, name: name, description: description, price: price
				})}
			>
	...
	if (menuItem) {
		return (
			<form
				className="item-form"
				onSubmit={(e) => handleItemSubmit(e, { iid: menuItem._id, catid: menuItem.catid, name: name, description: description, price: price })}
			>
				<label>Item Name</label><br/>
				<input
					type="text"
				...
				<input
					...
					onChange={(e) => setPrice(e.target.value)}
				/><br/>
				<button onClick={() => toggleEditItem(menuItem.catid, menuItem._id)}>
					Cancel
				</button>
				<button
					onClick={() => deleteItem(menuItem.catid, menuItem._id)}
